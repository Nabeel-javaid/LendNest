...
describe('ReputationManager', () => {
  ...
  describe('getDelinquentLoanIds', () => {
    it('should update account reputation before returning values', async () => {
      const { tx, LendNestV2, bidId } = await acceptedBid({ borrower, lender })
  ...
  describe('_applyReputation', () => {
    ...
    */
    ...
    it('should ensure bid id are removed when fully repaid', async () => {
      const { tx, LendNestV2, bidId, lendingToken } = await acceptedBid({
        borrower,
        lender,
      })
      ...
      await lendingToken.connect(borrower).approve(LendNestV2.address, totalOwed)

      const repayTx = await LendNestV2.connect(borrower).repayLoanFull(bidId)

      const activeBidIdsPostRepay = await LendNestV2.getBorrowerActiveLoanIds(
        borrowerAddress
      )
