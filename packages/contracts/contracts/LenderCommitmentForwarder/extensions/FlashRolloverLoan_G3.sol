...
import "../../interfaces/ILendNestV2Storage.sol";
...
contract FlashRolloverLoan_G3 is IFlashLoanSimpleReceiver, IFlashRolloverLoan {
    ...
    ILendNestV2 public immutable LendNest_V2;
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
     ...
     *
     * @param _LendNestV2 The address of the LendNestV2 contract.
    ...
    constructor(
        ...
        LendNest_V2 = ILendNestV2(_LendNestV2);
    ...
    function rolloverLoanWithFlash(
        ...
        address borrower = LendNest_V2.getLoanBorrower(_loanId);
        require(borrower == msg.sender, "CommitmentRolloverLoan: not borrower");

        // Get lending token and balance before
        address lendingToken = LendNest_V2.getLoanLendingToken(_loanId);
        ...
        // Call 'Flash' on the vault to borrow funds and call LendNestV2FlashCallback
     ...
     *      the repayment amount to the LendNestV2 contract and finally repays the loan in full.
     *
    ...
    function _repayLoanFull(
        ...
        LendNest_V2.repayLoanFull(_bidId);
    ...
    function calculateRolloverAmount(
        ...
        Payment memory repayAmountOwed = LendNest_V2.calculateAmountOwed(
            _loanId,
            _timestamp
        );
    ...
    function _getMarketFeePct(uint256 _marketId)
        ...
        returns (uint16)
    {
        address _marketRegistryAddress = ILendNestV2Storage(address(LendNest_V2))
            .marketRegistry();
    ...
    /**
     * @notice Fetches the protocol fee percentage from the LendNest V2 protocol.
     * @return The protocol fee percentage as defined in the LendNest V2 protocol.
     */
    function _getProtocolFeePct() internal view returns (uint16) {
        return IProtocolFee(address(LendNest_V2)).protocolFee();
    }
}
