...
contract CollateralManager is OwnableUpgradeable, ICollateralManager {
    ...
    /* Modifiers */
    modifier onlyLendNestV2() {
        require(_msgSender() == address(LendNestV2), "Sender not authorized");
        _;
    }
     ...
     * @param _LendNestV2 The address of the protocol.
     */
    function initialize(address _collateralEscrowBeacon, address _LendNestV2)
        external
        initializer
    {
        collateralEscrowBeacon = _collateralEscrowBeacon;
        LendNestV2 = ILendNestV2(_LendNestV2);
    ...
    ) public onlyLendNestV2 returns (bool validation_) {
        address borrower = LendNestV2.getLoanBorrower(_bidId);
     ...
     */
    function deployAndDeposit(uint256 _bidId) external onlyLendNestV2 {
    ...
    /**
     * @notice Gets the collateral asset amount for a given bid id on the LendNestV2 contract.
     * @param _bidId The ID of a bid on LendNestV2.
    ...
    function withdraw(uint256 _bidId) external {
        BidState bidState = LendNestV2.getBidState(_bidId);

        require(bidState == BidState.PAID, "collateral cannot be withdrawn");

        _withdraw(_bidId, LendNestV2.getLoanBorrower(_bidId));
     ...
     * @param _bidId The id of the bid to withdraw collateral for.
     */
    function lenderClaimCollateral(uint256 _bidId) external onlyLendNestV2 {
        if (isBidCollateralBacked(_bidId)) {
            BidState bidState = LendNestV2.getBidState(_bidId);
            ...
            _withdraw(_bidId, LendNestV2.getLoanLender(_bidId));
            emit CollateralClaimed(_bidId);
        }
    }
        ...
        returns (address proxyAddress_, address borrower_)
    {
        ...
        // Get bid info
        borrower_ = LendNestV2.getLoanBorrower(_bidId);
