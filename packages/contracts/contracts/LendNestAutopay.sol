...
import "./interfaces/ILendNestV2Autopay.sol";
...
import { Payment } from "./LendNestV2Storage.sol";

/**
 * @dev Helper contract to autopay loans
 */
contract LendNestV2Autopay is OwnableUpgradeable, ILendNestV2Autopay {
    using SafeERC20 for ERC20;
    using NumbersLib for uint256;

    ILendNestV2 public immutable LendNestV2;
    ...
    constructor(address _protocolAddress) {
        LendNestV2 = ILendNestV2(_protocolAddress);
    }
    ...
    function setAutoPayEnabled(uint256 _bidId, bool _autoPayEnabled) external {
        require(
            _msgSender() == LendNestV2.getLoanBorrower(_bidId),
    ...
    function autoPayLoanMinimum(uint256 _bidId) external {
        ...
        address lendingToken = ILendNestV2(LendNestV2).getLoanLendingToken(_bidId);
        address borrower = ILendNestV2(LendNestV2).getLoanBorrower(_bidId);
        ...
        // Approve the lendingToken to LendNestV2
        ERC20(lendingToken).approve(address(LendNestV2), amountToRepayMinimum);

        // Use that lendingToken to repay the loan
        LendNestV2.repayLoan(_bidId, amountToRepayMinimum);
    ...
    {
        Payment memory estimatedPayment = LendNestV2.calculateAmountDue(
            _bidId,
            _timestamp
        );
